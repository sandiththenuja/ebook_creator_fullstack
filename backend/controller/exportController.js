...
const exportAsDocument = async(req, res) => {
    try {
        ...
        // coverpage with image if available
        const coverPage = []
        ...
        if(book.coverImage && !book.coverImage.includes("pravatar")){  
            ...
            try {
                ...
                console.error(`Couldn't embed image: ${imagePath}`, imgErr.message);
        ...
        book.chapters.forEach((chapter, index) => {
            try {
                // page break before each chapter except first page
                if(index > 0){
                    sections.push(
                        new Paragraph({
                            text: "",
                    ...
                    new Paragraph({
                        children: [
                            new TextRun({
                                ...
                                bold: true,
                                font: DOCX_STYLES.fonts.heading,
                                size: DOCX_STYLES.sizes.chapterTitles * 2,
                                color: "1A202C"
                            })
                        ],
                        spacing: {
        ...
        if(!res.headersSent){
            res.status(500).json({message: "Server error while document export", error: error.message})
        }
    }
}
...
const exportAsPDF = async(req, res) => {
    try {
        ...
        // covet page with imagw if available
        if(book.coverImage && !book.coverImage.includes("pravatar")){
            const imagePath = book.coverImage.substring(1)
            ...
            try {
                ...
                console.error(`Couldn't embed image: ${imagePath}`, imgErr);
            }
        }
        ...
        if(!res.headersSent){
            res.status(500).json({message: "Server error for PDF export", error: error.message})
        }
    }
}
